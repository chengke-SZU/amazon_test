...
filename="main_xgboos_count_3D" # nam prefix
#model = linear_model.LogisticRegression(C=3)  # the classifier we'll use

model=xg.XGBoostClassifier(num_round=500 ,nthread=25,  eta=0.012, gamma=0.1,max_depth=30, min_child_weight=0.1, subsample=0.9, 
                               colsample_bytree=0.08,objective='binary:logistic',seed=1) 
# === load data in memory === #
print ("loading data")

y, X,X_test = load_datacount3D('train.csv','test.csv')
